		...
		<block v-for="(item, index) in formList" :key="index">
			...
			<block v-if="item.type == 'picker'">
				...
				<block v-if="item.range[item.rangeIndex] && item.range[item.rangeIndex].includes && item.range[item.rangeIndex].includes.length > 0">
					<register-form-a :rules.sync="item.range[item.rangeIndex].includes" @branch-update="branchUpdate"></register-form-a>
				</block>
			</block>
			<!-- 文本框 -->
...
<script>
	import RegisterFormA from './register_form_a.vue';
	export default {
		name: "register-form-b",
		props: {
			rules: {
				type: Array
			}
